...
**25.3 A front carries the name.** Nothing else. Earlier passes printed
the city and height, then the representation, on the front because the
record had them. The city was ambiguous (hers, or an office?), the height
duplicated the back's stats line, and the representation line was one more
thing competing with the photograph. The back is the document: measurements
in the agency order, and the address a booker can act on. The front is the
photograph and the name.

**25.4 Align type by ink, and test the layout as a system.** Type placed
by its line box sits visibly off; the Cover Story fits each word to the
trims from measured glyph extents in the browser (side bearings and
ascent from the canvas, the baseline set so the ink lands on the margin).
And a layout that only works for one name and one photograph is a picture
of a card, not a design: `render.cjs --cover-tests` renders the template
over long, short, hyphenated and diacritic names, a single name, and dark,
high-key, headshot and studio-colour frames. Look at that sheet before
calling a direction finished.

**25.5 The name can be part of the picture.** The second card's first
direction was a white rail with the name set vertically beside a
photograph module. It was resolved, and it was still a caption beside a
picture. The direction that earned its place puts the photograph on the
whole page and makes the name the only intervention: display grotesque at
trim-to-trim scale, the surname across the foot *behind* the figure so her
stride crosses the letters, the given name at the head on the darker
stone, each word taking light or dark ink from the luminance beneath it.
The figure layer is a stored matte (`scripts/comp-card-fronts/assets/`,
made by `matte.cjs` with an ad hoc matting model, never a dependency).
What was tried and dropped on the way: inverted (difference-blend) type,
which turns muddy blue on a brown floor; outlined type, which is a
texture, not a decision; a surname larger than the page, which cut the
name into "SZKO"; and black type on the white columns, which vanished
